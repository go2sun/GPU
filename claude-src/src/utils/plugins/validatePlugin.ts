...
export async function validatePluginManifest(
  ...
  // keys). We strip them here so the .passthrough() call below doesn't double-
  ...
  // We call .passthrough() locally here even though the base schema is lenient —
  ...
  const result = PluginManifestSchema().passthrough().safeParse(toValidate)
...
export async function validateMarketplaceManifest(
  ...
  // schema with .passthrough() here. Note .passthrough() on the outer object does NOT
  ...
  const strictMarketplaceSchema = PluginMarketplaceSchema()
    .extend({
      plugins: z.array(PluginMarketplaceEntrySchema().passthrough()),
    })
    .passthrough()
